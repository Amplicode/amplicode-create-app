...
const checkRequiredArgs = () => {
  ...
  ["appName", "template", "serverUrl"].forEach((required) => {
    if (!Object.keys(argv).includes(required)) {
      console.log(
        `Required argument "${required}" is not provided. Type --help for more information.`
      );
      process.exit(1);
    }
  });
...
const processTemplate = () => {
  const templateData = {
    ...
    ...Object.fromEntries(
      Object.entries(DEFAULT_VERSIONS).map(
        ([varName, { defaultVersion, packageName }]) => {
          try {
  ...
  const files = collectFilesRecursive(templateDir).map((filePath) =>
